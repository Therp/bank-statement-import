...
class PontoInterface(models.AbstractModel):
    ...
    def _login(self, username, password):
        ...
        _logger.debug("POST request on %s", url)
    ...
    def _set_access_account(self, access_data, account_number):
        ...
        _logger.debug("GET request on %s", url)
    ...
    def _get_transactions_from_data(self, data):
        ...
        if not transactions:
            _logger.debug(
                "No transactions where found in data %s",
                data,
            )
        else:
            _logger.debug(
                "%d transactions present in response data",
                len(transactions),
            )
        return transactions
    ...
    def _get_request(self, access_data, url, params):
        ...
        _logger.debug(
            "GET request to %s with headers %s and params %s", url, params, headers
        )
    ...
    def _get_response_data(self, response):
        ...
        _logger.debug("HTTP answer code %s from Ponto", response.status_code)
